...
use rce_engine::{api, config, debug, environment, run, unix_stream};
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        App::new().app_data(data.clone()).service(index_api).service(version_api).service(run_api)
    })
...
async fn index_api() -> HttpResponse {
    api::root::handle().map_or_else(prepare_error_response, prepare_success_response)
}

#[get("/version")]
async fn version_api(req: HttpRequest, config: web::Data<config::Config>) -> HttpResponse {
    if has_valid_access_token(&req, &config) {
        api::version::handle(&config).map_or_else(prepare_error_response, prepare_success_response)
    } else {
...
async fn run_api(
    ...
    if has_valid_access_token(&req, &config) {
        api::run::handle(&config, req_body.into_inner())
            .map_or_else(prepare_error_response, prepare_success_response)
    } else {
        prepare_error_response(api::authorization_error())
    }
}
...
fn prepare_success_response(data: api::SuccessResponse) -> HttpResponse {
    ...
    HttpResponse::build(status_code).content_type(ContentType::json()).body(data.body)
}
...
fn prepare_error_response(data: api::ErrorResponse) -> HttpResponse {
    ...
    HttpResponse::build(status_code).content_type(ContentType::json()).body(body)
}

fn has_valid_access_token(request: &HttpRequest, config: &config::Config) -> bool {
    let access_token =
        request.headers().get("X-Access-Token").map(|token| token.to_str().unwrap_or(""));
...
fn build_config(env: &environment::Environment) -> Result<config::Config, environment::Error> {
    ...
    Ok(config::Config { server, api, unix_socket, container, run, debug })
}
...
fn build_server_config(
    ...
    Ok(config::ServerConfig { listen_addr, listen_port, worker_threads })
}
...
fn build_container_config(
    ...
    Ok(run::ContainerConfig {
        ...
        readonly_rootfs,
        tmp_dir: tmp_dir_path.map(|path| run::Tmpfs { path, options: tmp_dir_options }),
        work_dir: work_dir_path.map(|path| run::Tmpfs { path, options: work_dir_options }),
    })
}
...
fn build_run_config(env: &environment::Environment) -> Result<run::Limits, environment::Error> {
    ...
    Ok(run::Limits { max_execution_time: Duration::from_secs(max_execution_time), max_output_size })
}

fn build_debug_config(env: &environment::Environment) -> debug::Config {
    let keep_container = environment::lookup(env, "DEBUG_KEEP_CONTAINER").unwrap_or(false);

    debug::Config { keep_container }
}
